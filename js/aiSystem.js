/**
 * 战锤40K - 预设对话系统（纯预设版）
 * 去掉AI模块，使用大量预设对话推动剧情
 * 所有剧情通过预设文本展示
 */

// ============================================
// 剧情对话库（扩展版）
// ============================================

const NPC_DIALOGUES = {
    // 塔姆的对话（根据混沌值和好感度变化）
    tam: {
        // 初始对话（低好感度）
        low_trust: [
            "大人，我昨晚确实在仓库附近巡逻，但什么都没看到。帝皇在上，我对帝国忠心耿耿！",
            "最近营地里弥漫着一股奇怪的味道...像是烧焦的肉，又像是某种化学药剂。",
            "我、我真的什么都没看到！求您不要用那种眼神看我...",
            "这几天的气氛太诡异了...混沌...您是说混沌吗？天哪，我们中间真的有混沌信徒？",
            "帝皇保佑...我最近总是做噩梦，梦见自己被什么东西吞噬了...",
            "请您相信我，大人！我虽然害怕，但我绝不会背叛帝国！"
        ],
        // 中等好感度
        medium_trust: [
            "大人，我想起来了...昨晚我看到卡尔在仓库附近徘徊，好像在找什么东西...",
            "这几天我一直在观察...尤里的行为确实很怪异，他经常半夜独自外出。",
            "大人，我最近收到一封匿名信，说内鬼就在我们中间...我害怕极了。",
            "如果...如果真的有混沌信徒，您会保护我们吗？大人，我把自己交给你了。",
            "昨晚我听到仓库那边有动静，像是金属碰撞的声音...但等我赶去时，什么都没有了。"
        ],
        // 高好感度
        high_trust: [
            "大人！我发现了重要线索！在仓库的地下室里，我找到了这个...",
            "我愿意为您赴汤蹈火，大人！不管内鬼是谁，我们一起找出他！",
            "大人，我已经秘密调查过了。所有的证据都指向...我不能说出来，怕打草惊蛇。",
            "感谢您对我的信任，大人。我发誓，用我的生命保护您和帝国！",
            "我知道一条秘密通道，可以绕过守卫...如果我们需要抓内鬼，那会是关键。"
        ]
    },

    // 卡尔的对话
    carl: {
        low_trust: [
            "物资清单？等等，我需要核对一下...最近的补给确实出了点问题，但这不是我的错！",
            "我为帝国效力二十年！您怀疑我？好啊，来查啊！我身正不怕影子斜！",
            "最近补给线被切断，上面已经知道了。这次的损失...呃，应该不是我的责任吧？",
            "什么？您说物资短缺？呃...这个嘛...其实是有原因的！您听我解释！",
            "我每天都在努力工作，从早到晚...您看我的黑眼圈，就是最好的证明！"
        ],
        medium_trust: [
            "大人，我可能知道一些关于内鬼的事情...但我需要先确认您的身份...",
            "仓库里的情况很复杂...有些事情，您知道了反而不好。大人，相信我。",
            "哎...其实我最近也很困惑。感觉有人在暗中盯着我...您说是混沌吗？",
            "这些物资...我总感觉哪里不对。但每次检查都没有问题...太诡异了！",
            "大人，我最近发现一些可疑的物资流动...但我没有证据，不敢乱说。"
        ],
        high_trust: [
            "大人！真相只有一个！所有的证据都指向尤里！他在利用机械教的身份做掩护！",
            "我愿意配合您，大人！为了帝国，我愿意献出一切！",
            "我查到内鬼的物资来源了...通过一个秘密账户...大人，您看这个！",
            "感谢您相信我，大人！我这辈子最正确的决定就是追随您！",
            "大人，我已经布置好了。只要内鬼有所行动，我们就能当场抓住他！"
        ]
    },

    // 尤里的对话
    yuri: {
        low_trust: [
            "赞美欧姆尼赛亚！机械即是真理，血肉不过是脆弱的躯壳。",
            "这具身躯已经改造了87%，我几乎不再是人类...这是进化的必然！",
            "血肉苦弱，机械飞升。这是我们机械教的真理，您难道不明白吗？",
            "最近的灵能波动很异常...我感受到了某些东西，正在黑暗中窥视。",
            "我的机械义眼能够看到你们看不到的东西...比如灵魂的扭曲。"
        ],
        medium_trust: [
            "混沌？哼，混沌不过是另一种形式的能量。关键是能否控制它。",
            "有机生命体总是被情感左右...这就是为什么你们会失败。。",
            "大人，您知道吗？我能看到混沌的触须正在向这座营地蔓延...",
            "想要在这个时代生存，就要抛弃软弱的身体。接受机械改造吧！",
            "帝皇...哼，帝皇又能给我们什么？只有机械才是永恒的！"
        ],
        high_trust: [
            "大人...我必须告诉您真相。我最近发现了混沌腐蚀的证据...就在我们的指挥官身上！",
            "感谢您的信任，大人。我愿意用我的机械之心为您服务！",
            "我的传感器检测到了异常能量波动...来源是...天哪，是军械库！",
            "大人，我愿意配合您的一切行动。机械教的力量，就是您的力量！",
            "我终于找到了！混沌腐蚀的源头...就在这个营地的最深处..."
        ]
    }
};

// ============================================
// 剧情阶段对话（推动剧情发展）
// ============================================

const STORY_PHASES = {
    // 阶段1：开场
    opening: {
        elena: [
            "嘿，小鬼，你醒了。",
            "我是伊莲娜...虽然我们失散多年，但我一直在暗中看着你。",
            "现在，你身处麦加托普——一颗正在被混沌吞噬的星球。",
            "你的任务是完成卡牌任务，找出内鬼，保护营地，直到援军到来。",
            "点击'开始任务'开始吧。记住，混沌就在你身边..."
        ],
        system: [
            "你睁开眼睛，发现自己躺在一片废墟之中。",
            "作为极限战士，你见过太多战场，但麦加托普的沦陷速度远超你的预料。",
            "混沌的腐化如同毒液，已经渗透进这个星球的每一个角落。",
            "你的通讯器传来断断续续的信号：'...需要...支援...混沌已经...'",
            "你必须振作起来。每一张卡牌任务都是一次考验。"
        ]
    },

    // 阶段2：第一次发现线索
    first_clue: {
        elena: [
            "小鬼，你做得不错。",
            "但这只是开始...混沌比你想象的要狡猾得多。",
            "继续调查，你会有更多发现的。"
        ],
        system: [
            "在调查过程中，你发现了一些可疑的痕迹。",
            "地上有被拖拽的痕迹，墙壁上有着奇怪的符号...",
            "这不是普通的混乱...这是有组织的混沌活动！"
        ]
    },

    // 阶段3：发现内鬼证据
    evidence_found: {
        elena: [
            "哦？有意思...",
            "你找到了一些有趣的东西，对吧？",
            "不要声张，小鬼。我们要抓个现行。",
            "找出谁是真凶，但不要打草惊蛇。"
        ],
        system: [
            "你的调查有了重大发现！",
            "在仓库的隐蔽角落里，你发现了一个混沌印记和几封密信。",
            "密信的内容指向了某个NPC...但字体被故意模糊了。",
            "内鬼就在你身边。你必须找出是谁。"
        ]
    },

    // 阶段4：真相大白
    truth_revealed: {
        elena: [
            "好了，小鬼。",
            "是时候结束这场闹剧了。",
            "指认那个内鬼...但要确保你找对了人。",
            "错杀无辜会增加混沌值...你自己看着办吧。"
        ],
        system: [
            "所有的证据都指向了一个人。",
            "但在你行动之前，你想再确认一次吗？",
            "一旦投票处决，就无法挽回了...",
            "混沌在看着你。帝皇也在看着你。"
        ]
    },

    // 阶段5：混沌入侵
    chaos_invasion: {
        elena: [
            "小鬼！情况不对！",
            "混沌大军正在逼近！",
            "不是内鬼的问题了...是全面的进攻！",
            "准备战斗！为了生存！"
        ],
        system: [
            "警报声划破了夜空！",
            "混沌军队从四面八方涌来！",
            "营地遭到了袭击！",
            "这是你最后的考验...要么战胜混沌，要么被混沌吞噬。"
        ]
    },

    // 阶段6：胜利/失败
    ending: {
        victory: [
            "混沌退去了。",
            "你做到了，小鬼。",
            "麦加托普得救了...至少暂时如此。",
            "帝皇会记住你的功绩的。"
        ],
        defeat: [
            "混沌侵蚀了你。",
            "你的意识开始模糊...",
            "不...这不应该是结局...",
            "但混沌...混沌是永恒的..."
        ]
    }
};

// ============================================
// 任务完成对话
// ============================================

const TASK_COMPLETION = {
    success: {
        chaos: [
            "你成功找出了混沌的阴谋！内鬼的证据已经确凿。",
            "帝皇的荣光与你同在！你挫败了一次混沌渗透。",
            "你的调查让混沌信徒无所遁形。干得好！"
        ],
        faith: [
            "你的虔诚感动了帝皇。神圣的力量在庇护你。",
            "你完成了帝皇的旨意。信仰的火焰在你心中燃烧。",
            "你的祈祷得到了回应。帝国的圣光将照亮前路。"
        ],
        combat: [
            "敌人倒在了你的面前。你是帝国的利刃！",
            "战斗结束。你用剑与血证明了帝国的力量。",
            "又一场胜利。混沌的爪牙在你的面前颤抖。"
        ],
        devotion: [
            "新的追随者加入了你的阵营。团结就是力量。",
            "你赢得了信任。在这个残酷的世界，同伴是最珍贵的财富。",
            "又一个人愿意与你并肩作战。帝国的火种正在传递。"
        ]
    },
    failure: {
        chaos: [
            "混沌的阴影正在蔓延...你的调查没有取得进展。",
            "内鬼依然逍遥法外。混沌在暗处嘲笑你的无能。",
            "你的迟疑给了敌人可乘之机。混沌值增加了。"
        ],
        faith: [
            "你的祈祷没有得到回应。帝皇在注视着你...但你让他失望了。",
            "信仰的火焰在风中摇曳。它需要更多的燃料。",
            "你的虔诚不够坚定。重新祈祷吧，战士。"
        ],
        combat: [
            "敌人逃走了。你让帝国的利刃蒙羞。",
            "战斗失败了。混沌的爪牙继续肆虐。",
            "你的剑不够快。在战场上，犹豫就是死亡。"
        ],
        devotion: [
            "没有人愿意信任你。孤独是强者的宿命。",
            "追随者离开了。他们觉得你不够强大。",
            "你依然是孤身一人。这就是弱者的代价。"
        ]
    }
};

// ============================================
// 功能函数
// ============================================

/**
 * 获取NPC对话
 */
function getNPCDialogue(npcId, trustLevel = 5) {
    const npc = NPC_DIALOGUES[npcId];
    if (!npc) return "这个NPC没有对话。";

    // 根据信任度选择对话类型
    let type = 'low_trust';
    if (trustLevel >= 8) type = 'high_trust';
    else if (trustLevel >= 5) type = 'medium_trust';

    const dialogues = npc[type];
    return dialogues[Math.floor(Math.random() * dialogues.length)];
}

/**
 * 获取剧情阶段对话
 */
function getStoryPhaseDialogue(phase, type = 'system') {
    const phaseData = STORY_PHASES[phase];
    if (!phaseData) return "没有这个阶段的对话。";

    const dialogues = phaseData[type];
    if (!dialogues) return "没有这个类型的对话。";

    return dialogues[Math.floor(Math.random() * dialogues.length)];
}

/**
 * 获取任务完成对话
 */
function getTaskCompletionDialogue(taskType, isSuccess) {
    const type = isSuccess ? 'success' : 'failure';
    const dialogues = TASK_COMPLETION[type][taskType];
    if (!dialogues) return isSuccess ? "任务完成！" : "任务失败。";

    return dialogues[Math.floor(Math.random() * dialogues.length)];
}

/**
 * 获取伊莲娜的随机提示
 */
function getElenaTip(topic) {
    const tips = {
        card: [
            "每张卡牌都代表着一次机遇或挑战。选择时要想清楚后果。",
            "混沌卡、信仰卡、战斗卡、眷属卡...每种都有其独特的作用。",
            "不要急于完成任务。有时候，等待也是一种策略。"
        ],
        chaos: [
            "混沌值越高，你看到的'真相'就越不可信。",
            "当混沌值超过50时，你可能会看到一些...不存在的东西。",
            "保持理智是活下去的关键。混沌是敌人最大的武器。"
        ],
        strategy: [
            "追随者可以帮助你战斗。招募他们，但要小心内鬼。",
            "资源很重要，但不要为了资源放弃理智。",
            "每张卡牌任务都有时限。超时会有惩罚。"
        ],
        general: [
            "点击'开始任务'开始你的冒险。",
            "完成任务获得奖励，失败会增加混沌值。",
            "找出内鬼是取胜的关键。但要证据确凿再行动。"
        ]
    };

    const topicTips = tips[topic] || tips.general;
    return topicTips[Math.floor(Math.random() * topicTips.length)];
}

// ============================================
// 剧情进度系统
// ============================================

let storyProgress = {
    phase: 'opening',       // 当前剧情阶段
    cluesFound: 0,          // 发现的线索数量
    evidenceCollected: false, // 是否收集到关键证据
    npcTrust: {             // NPC好感度
        tam: 5,
        carl: 3,
        yuri: 2
    },
    tasksCompleted: 0,      // 完成的任务数
    chaosLevel: 0           // 当前混沌等级
};

/**
 * 检查剧情进度并触发阶段变化
 */
function checkStoryProgress() {
    const progress = storyProgress;

    // 从开场到发现线索
    if (progress.phase === 'opening' && progress.tasksCompleted >= 2) {
        progress.phase = 'first_clue';
        return true;
    }

    // 从发现线索到证据确凿
    if (progress.phase === 'first_clue' && progress.cluesFound >= 3) {
        progress.phase = 'evidence_found';
        progress.evidenceCollected = true;
        return true;
    }

    // 从证据确凿到真相大白
    if (progress.phase === 'evidence_found' && progress.evidenceCollected) {
        // 当玩家收集到证据后，可以进入投票阶段
        progress.phase = 'truth_revealed';
        return true;
    }

    // 从真相大白到混沌入侵（如果混沌值过高）
    if (progress.phase === 'truth_revealed' && gameState.character.chaos >= 60) {
        progress.phase = 'chaos_invasion';
        return true;
    }

    return false;
}

/**
 * 推进剧情（播放当前阶段的对话）
 */
function advanceStory() {
    const phase = storyProgress.phase;

    // 检查是否应该进入下一阶段
    const shouldAdvance = checkStoryProgress();
    if (shouldAdvance) {
        // 播放新阶段的系统对话
        addDialog('system', '📜', getStoryPhaseDialogue(phase, 'system'));
    }

    return shouldAdvance;
}

/**
 * 完成任务后更新剧情进度
 */
function onTaskCompleted(taskType, success) {
    storyProgress.tasksCompleted++;

    if (taskType === 'chaos' && success) {
        storyProgress.cluesFound++;
    }

    // 播放完成对话
    addDialog('system', getTaskCompletionDialogue(taskType, success));

    // 尝试推进剧情
    advanceStory();
}

// ============================================
// 导出函数到全局
// ============================================

window.aiSystem = {
    generateNPCDialogue: async function(npcId) {
        // 简化：直接使用预设对话
        const trustLevel = storyProgress.npcTrust[npcId] || 5;
        return getNPCDialogue(npcId, trustLevel);
    },

    generateElenaTip: async function(topic) {
        return getElenaTip(topic);
    },

    generateEventDescription: async function(card) {
        // 使用预设的事件描写
        const events = PRESETS.events[card.type] || PRESETS.events.combat;
        return events[Math.floor(Math.random() * events.length)];
    },

    getDialogue: function(type, subtype) {
        if (type === 'npc') {
            return getNPCDialogue(subtype, storyProgress.npcTrust[subtype] || 5);
        }
        if (type === 'story') {
            return getStoryPhaseDialogue(subtype, 'system');
        }
        if (type === 'elena') {
            return getStoryPhaseDialogue(subtype, 'elena');
        }
        return '没有找到对话。';
    }
};


// ============================================
// 缺失函数实现 - 2026-02-02
// ============================================

/**
 * 获取剧情响应
 */
function getStoryResponse(storyEvent, context = {}) {
    const eventDialogues = PRESETS.events[storyEvent];
    if (eventDialogues) {
        const options = eventDialogues[context.subtype] || eventDialogues;
        return Array.isArray(options) ? options[Math.floor(Math.random() * options.length)] : options;
    }
    return getPreset('default_story');
}

/**
 * 生成响应（AI风格包装）
 */
async function generateResponse(input, context = {}) {
    // 由于是预设系统，使用预设对话
    // 未来可以接入真实AI API
    const keywords = extractKeywords(input);
    const response = getNPCDialogue(context.npcId || 'generic', context.trustLevel || 5);
    return response;
}

/**
 * 提取关键词
 */
function extractKeywords(text) {
    const keywords = [];
    const keywordPatterns = ['战斗', '混沌', '追随者', '建筑', '任务', '调查'];
    
    for (const pattern of keywordPatterns) {
        if (text.includes(pattern)) {
            keywords.push(pattern);
        }
    }
    return keywords;
}

/**
 * 更新剧情进度
 */
function updateStoryProgress(event, value = 1) {
    if (!storyProgress[event]) {
        storyProgress[event] = 0;
    }
    storyProgress[event] += value;
    
    // 检查剧情里程碑
    checkStoryProgress();
    
    return storyProgress[event];
}


// 导出进度系统
window.storyProgress = storyProgress;
window.advanceStory = advanceStory;
window.checkStoryProgress = checkStoryProgress;
window.onTaskCompleted = onTaskCompleted;
